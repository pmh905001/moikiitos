...
public class UserControllerTest {
	...
	public void duplicateUsernameShouldBeInvalid() {
		...
		when(mockUserService.userExists(USERNAME,null)).thenReturn(true);
		...
		assertEquals(expectedModel, model);
		verify(mockUserService).userExists(USERNAME,null);
		verify(mockBindingResult).hasErrors();
	}
	...
	public void uniqueUsernameShouldBeValid() {
		...
		UserService mockUserService = mock(UserService.class);
		when(mockUserService.userExists(USERNAME,null)).thenReturn(false);
		// no error messages returned
		when(mockUserService.registerUser(USERNAME, PASSWORD,null)).thenReturn("");
		...
		verify(mockUserService).userExists(USERNAME,null);
		verify(mockUserService).registerUser(USERNAME, PASSWORD,null);
		verify(mockBindingResult).hasErrors();
	}
	...
	public void createUserErrorIsReported() {
		...
		UserService mockUserService = mock(UserService.class);
		when(mockUserService.userExists(USERNAME,null)).thenReturn(false);
		when(mockUserService.registerUser(USERNAME, PASSWORD,null)).thenReturn(errorMessage);
